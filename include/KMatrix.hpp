 ...
 */
template <arma::uword numChannels, arma::uword numAlphas>
class KMatrix {
  public:
    arma::cx_fmat mAlphas = arma::cx_fmat::fixed<1, numAlphas>();
    arma::cx_fmat mChannels = arma::cx_fmat::fixed<numChannels, 2>();
    arma::cx_fvec m1s = arma::cx_fvec::fixed<numChannels>();
    arma::cx_fvec m2s = arma::cx_fvec::fixed<numChannels>();
    arma::cx_fmat gAlphas = arma::cx_fmat::fixed<numChannels, numAlphas>();
    arma::cx_fmat cBkg = arma::cx_fmat::fixed<numChannels, numChannels>();
    int J;
    arma::fmat bwAlphaMat = arma::fmat::fixed<numChannels, numAlphas>();
    arma::fcube bwAlphaCube = arma::fcube::fixed<numChannels, numChannels, numAlphas>();
    ...
    arma::fmat B(const float& s) const {
      arma::fmat::fixed<numChannels, numAlphas> result(arma::fill::zeros);
    ...
    arma::fcube B2(const float& s) const {
      arma::fcube::fixed<numChannels, numChannels, numAlphas> result(arma::fill::zeros);
    ...
    arma::cx_fmat K(const float& s) const {
      arma::cx_fmat::fixed<numChannels, numChannels> result(arma::fill::zeros);
      arma::cx_fcube::fixed<numChannels, numChannels, numAlphas> gigj(arma::fill::zeros);
      for (size_t k = 0; k < numAlphas; k++) {
...
template <arma::uword numChannels, arma::uword numAlphas>
KMatrix<numChannels, numAlphas>::KMatrix(int J) : J(J) {
  if (J == 0) {
